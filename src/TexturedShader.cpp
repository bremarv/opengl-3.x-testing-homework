...
static std::string normal_map_vs_src =
	"#version 130\n"
	"\n"
...
static std::string normalMap_fs_src =
	"#version 130\n"
	"\n"
	"in vec2 in_f_TexCoord;\n"
	"in vec3 in_f_ObjSpacePosition;\n"
	...
	"\n"
        ...
        "main()\n"
        "{\n"
		" vec3 norm = texture(DiffuseMap, in_f_TexCoord).xyz;\n"
...
TexturedShader::TexturedShader( ShaderType type ) :
...
void TexturedShader::init(const std::string &vs, const std::string &fs)
{
    ...
    m_diffusemap_sampler_loc =
            glGetUniformLocation( m_prog, "DiffuseMap" );
}


TexturedShader*
TexturedShader::factory( ShaderType type )
{
    TexturedShader* ret = new TexturedShader(type);
	try
	{
		ret->init();
	} catch (...)
...
void 
TexturedShader::setTexture(boost::shared_ptr<TextureHandle> tex)
{
	m_tex = tex;
}

void
TexturedShader::setUp( Object*  shape,
                      Viewer*  camera,
                      Light*   light )
{
	StandardShaderProgram::setUp(shape, camera, light);

    if( m_diffusemap_sampler_loc != -1 ) {
		glUniform1i( m_diffusemap_sampler_loc, DIFFUSE_MAP_TEXTURE_UNIT );
		glActiveTexture( GL_TEXTURE0 + DIFFUSE_MAP_TEXTURE_UNIT);
		if (m_tex)
			glBindTexture( GL_TEXTURE_2D, m_tex->getTex() );
	}
...
TexturedShader::~TexturedShader(void)
{
}
